...
const renewAccessToken = async (req, res) => {
    ...
    try {
        ...
        const { institute_user, exp } = jwt.decode(data)
        ...
        const new_access_token = jwt.sign(institute_data, ACCESS_TOKEN_SECRET_KEY, { expiresIn: user_tokens_expires_in.access_user_tokens_expires_in })
        const new_refresh_token = jwt.sign(institute_data, REFERESH_TOKEN_SECRET_KEY, { expiresIn: user_tokens_expires_in.refresh_user_tokens_expires_in })

        let find_tokens = await Institute_access_refresh_tokens.findOne({ refresh_token: data, access_token: headers_token })
        ...
        let updated_tokens = await Institute_access_refresh_tokens.findByIdAndUpdate(_id, { access_token: new_access_token, refresh_token: new_refresh_token, renew_token: true }, { new: true })

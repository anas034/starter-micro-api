...
const get_batches_by_course_id = async (req, res) => {
    ...
    try {
        ...
        const find_batches = await createBatch.find({ course_id })
        ...
        if (find_batches.length) {
            ...
            for (let index = 0; index < find_batches.length; index++) {
                ...
                const find_institute = await createInstitute.findById(find_batches[index].institute_id)
            ...
            await res.send({ success: true, data: baches_array }).status(200)
            return
        }
        else {
            await res.send({ success: true, data: [] }).status(200)
            return
        }
...
module.exports = get_batches_by_course_id
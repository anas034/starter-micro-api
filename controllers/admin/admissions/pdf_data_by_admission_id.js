...
const pdf_data_by_admission_id = async (req, res) => {
    try {
        const { id } = req.params
        ...
        if (!is_valid_id) {
            return res.status(402).send({ success: false, message: "Admission id not valid!" })
        }
        ...
        if (!user_profile) {
            return res.status(402).send({ success: false, message: "User profile is no longer available!" })
        }
        if (!find_course) {
            return res.status(402).send({ success: false, message: "Course is no longer available!" })
        }
        if (!find_institute) {
            return res.status(402).send({ success: false, message: "Institute is no longer available!" })
        }
        if (!find_batch) {
            return res.status(402).send({ success: false, message: "Batch is no longer available!" })
        }

        const { city, parentsGuardianNumber, email, cnicBform, dateofBirth, permanentAddress, lastQualification, gender, rollNumber, time_slot, phoneNumber, qr_code } = find_admission
...
module.exports = pdf_data_by_admission_id